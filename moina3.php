<?php
	...
	3.8		
		enum{} accepts values
		extend publish over all files from hense declared
	3.6:
	...
	*/
	...
	function strposrev( $hey, $needle, $start=-1 ) {
		...
		return strrpos( substr( $hey, 0, $start+1), $needle ); 
	 }
		...
		while ( ( $i = strpos( $src, '<?php', $i+1 ) ) !== false ) {
				...
				foreach ( array_reverse( scan($php, '==>') ) as $q ) {													
					...
					if ( $php[$p] !== ')' ) { // naked: decl with no parameters
						...
						if ( preg_match( '/\W/', $php[$p] ) ) { //anonymous naked decl
							...
							if ( $php[$k+1] != '$' ) $prePHP->decls[] = substring($php, $k+1, $p ); //keep decl name to globalize it later
					...
					if ( $php[$p] == ')' ) { //decl with params
						...
						if ( preg_match( '/\W/', $php[$p-1] ) ) { //not preceded with function name?
							...
							for( $k = $q+3; $php[$k] <= ' '; $k++ ); //get body start
							if ( ord($php[$p-1]) < 128 ) {
								$prePHP->declf[] = substring( $php, $b+1, $p-1);
							...
							if ( $php[$k] == '{' ) { //long function:	func($x) ==> {
								...
								if ( $php[$b+1] == '$' ) { //function variable									
									...
									$n = 0;
									for( $m = $k;; $m++ ) {
										if ( ! isset( $php[$m] ) ) die('ERROR: missing ; in ' . unhide( substr( $php, $k, 20) ) );
										switch( $php[$m] ) {
			...
			//OOX: overload/override/extend function str():
				if ( preg_match_all( '/[\n\r]\s*(overload|override|extend)\s+(function)\s+(\w+.)/', 
					...
					$d = 0; //must be processed in order as in code
					foreach ( $M as $m ) {			
						$p = $m[1][1] + $d;					//   over_l function xxx( type $x ) 			
						$k = strpos( $php, '(', $p );	//   p                  k         q 
						$q = strpos( $php, ')', $k );	//                    fn     ps							
						$fn = $m[3][0];	
						if ( $fn[-1] == '(' ) $fn = substr( $fn, 0, -1);
						if ( $fn[-1] > '~' ) $fn[-1] = chr( ord( $fn[-1] ) & 127 );
			...
			//enum {}
				...
				foreach( $prePHP->enum ?? [] as $k => $v ) {
					$php = preg_replace( '/\\b'.$k.'\\b/', $v, $php );			
				}								

			//publish $X;  ...make it global from this file onwards
				// $X ===> $GLOBALS['X']
				$prePHP->published ??= [];
				...
				foreach( $prePHP->published as $fn ) {
	...
	function deref() { global $prePHP;
		foreach ( $prePHP->script as $f => $src ) {
			...
			while ( ( $i = strpos( $src, '<?php', $i+1 ) ) !== false ) {
				...
				foreach( $prePHP->decls as $fn ) 
					$php = preg_replace( '/\b'.$fn.'\b/', $fn.'()', $php );					

				
				if ( preg_match_all( '/\)\s*=[^=>]/', $php, $M, PREG_OFFSET_CAPTURE  ) ) {
					//instance($k) = 'harry';					
					foreach ( array_reverse( $M[0] ) as $m ) {
						$jj = $m[1];		
						$ii = strposrev( $php, "\n", $jj );
						...
						$kk = strpos( $php, '(', $ii );
						$mm = strpos( $php, ';', $jj );
						$fn = substring( $php , $ii, $kk-1 );						
						if ( in_array( $fn, $prePHP->decls ) || in_array( $fn, $prePHP->declf ) ) {
							$php = substring_replace( $php, 
								',' . substring( $php, $jj + strlen($m[0]), $mm - 1 ) . ');', 
								$jj, $mm );
						}
					}
				}
